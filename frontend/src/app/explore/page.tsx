    ...
    <div className="container mx-auto py-10 px-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Browse real-world failure cases and insights.
          </p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-64 bg-muted animate-pulse rounded-xl border"
            ></div>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredFailures.map((failure) => (
            <div
              key={failure.id}
              className="group relative glass p-10 h-full flex flex-col transition-all duration-500 hover-lift rounded-[2rem] border-white/5 bg-white/[0.02] animate-in fade-in slide-in-from-bottom-8 duration-700"
            >
              ...
              <div className="mt-auto pt-8 border-t border-white/5 flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-primary/60 mb-1">
                    Sector
                  </span>
      ...
      {!loading && filteredFailures.length === 0 && (
        <div className="text-center py-20">
          <p className="text-muted-foreground">
            No failures found matching your search.
          </p>
        </div>
      )}
    </div>
