...
export default function FailureDetailsPage() {
  ...
  if (!failure)
    return (
      <div className="container mx-auto py-20 text-center font-serif text-2xl">
  ...
  return (
    <div className="container mx-auto py-12 px-4 max-w-7xl font-sans relative">
      ...
      <Button
        variant="ghost"
        className="mb-12 hover:bg-primary/10 text-[10px] uppercase font-black tracking-widest gap-3 rounded-full px-6 transition-all"
        asChild
      >
        <Link href="/explore">
          ...
          <div className="glass p-12 rounded-[3rem] border-accent/20">
            ...
            <div className="space-y-16 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-[2px] before:bg-gradient-to-b before:from-primary/20 before:via-accent/20 before:to-transparent">
              {[
                {
                  year: failure.year - 2,
                  title: t("timelineCapital"),
                  desc: t("timelineCapitalDesc"),
                },
                {
                  year: failure.year - 1,
                  title: t("timelineMarket"),
                  desc: t("timelineMarketDesc"),
                },
                {
                  year: failure.year,
                  title: t("timelineTerminal"),
                  active: true,
                  desc: t("timelineTerminalDesc"),
                },
              ].map((item, i) => (
                <div
                  key={i}
                ...
                >
                  <div
                    className={`flex items-center justify-center w-12 h-12 rounded-2xl border ${item.active ? "bg-destructive/20 border-destructive text-destructive shadow-[0_0_20px_rgba(239,68,68,0.3)]" : "bg-primary/20 border-primary text-primary shadow-[0_0_20px_rgba(99,102,241,0.2)]"} z-10 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 transition-all duration-500 group-hover:scale-125`}
                  >
                    <Zap className="h-5 w-5" />
                  </div>
                  <div
                    className={`w-[calc(100%-4.5rem)] md:w-[45%] p-8 glass rounded-[2rem] border-white/5 hover:border-primary/40 transition-all duration-500 ${item.active ? "border-destructive/40" : ""}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div
                        className={`text-2xl font-black ${item.active ? "text-destructive" : "text-primary"}`}
                      >
                        {item.year}
                      </div>
                    </div>
                    <div
                      className={`text-[10px] font-black tracking-[0.3em] mb-3 ${item.active ? "text-destructive/80" : "text-primary/70"}`}
                    >
                      {item.title}
                    </div>
        ...
        <div className="lg:col-span-4 space-y-10 animate-in fade-in slide-in-from-right-12 duration-700 delay-300">
          <div className="glass-dark p-10 rounded-[2.5rem] border-primary/30 shadow-2xl neural-pulse">
            ...
            <p className="text-foreground/80 text-sm leading-relaxed mb-10 font-medium">
              Forensic vectors for{" "}
              <span className="text-foreground font-black">
                {failure.title}
              </span>
              {t("indicateStructural")}{" "}
              <span className="text-primary font-black uppercase tracking-widest">
                {failure.category} instability
              </span>{" "}
              during the {failure.year} {t("cycle")}.
            </p>
            <Button
              size="lg"
              className="w-full bg-primary text-primary-foreground hover:bg-primary/90 rounded-2xl h-16 text-[10px] font-black uppercase tracking-[0.3em] shadow-lg shadow-primary/20 border-none transition-all group"
              asChild
            >
              <Link href="/analyzer">
          ...
          <div className="glass p-10 rounded-[2.5rem]">
            <div className="text-[10px] uppercase tracking-[0.4em] font-black text-muted-foreground mb-10 border-b border-white/5 pb-6">
              Market Metrics
            </div>
            <div className="space-y-10">
              <div className="group">
                <div className="text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-2">
                  Saturation Load
                </div>
                <div className="text-xl font-black text-foreground tracking-tighter">
                  CRITICAL SATURATION
                </div>
                <div className="text-[9px] text-accent font-black uppercase mt-2 tracking-widest">
                  STATUS: VOLATILE
                </div>
              </div>
              <div className="group pt-8 border-t border-white/5">
                ...
                <div className="text-[9px] text-muted-foreground/40 font-black uppercase mt-2 tracking-widest">
                  ECOSYSTEM: {failure.industry}
                </div>
              </div>
            </div>
          </div>
